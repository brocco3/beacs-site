...
## v3.6 Performance会場表示
Google SheetsのSiteタブに次の行を追加します。

- Key: `history`
- Value例: `豊川稲荷妙厳寺｜伊佐須美神社｜北文化小劇場｜おおぶ文化交流の杜`

改行を入れた場合は、その改行もサイトへ反映されます。Eventsタブの「実績表示」は使用しません。


## v3.7 主な演奏会場
Eventsタブで「実績表示」をTRUE（チェック）にした行の「会場」を取得し、PERFORMANCE下部に「｜」区切りで表示します。SCHEDULE用の「表示」がFALSEでも対象になります。同じ会場は1回だけ表示します。
